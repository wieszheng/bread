...
@Software : PyCharm
"""
max_length = max(len(line.split('=')[0].strip()) for line in [
    "__table_args__",
    "id",
    "creation_date",
    "created_by",
...
formatted_lines = [
    ...
    format_line("id", "mapped_column(BigInteger(), nullable=False, primary_key=True, autoincrement=True)", "主键"),
    format_line("creation_date", "mapped_column(DateTime(), default=func.now())", "创建时间"),
    format_line("created_by", "mapped_column(BigInteger)", "创建人ID"),
    format_line("updation_date", "mapped_column(DateTime(), default=func.now(), onupdate=func.now())", "更新时间"),
